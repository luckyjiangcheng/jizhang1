...
*   确认“显示网页视图”的输出变量（建议命名 `FinalCSVLine`）**直接**进入“追加到文件”。
*   确认“追加到文件”使用的是 `FinalCSVLine`，而不是 `AIResult`、`请求输入`、`更新后的文本` 或网页页面文本。

如果出现你截图里的两个现象，请按对应方式处理：
*   **图1（确认后显示一行 CSV 文本）**：说明设备上的 `confirm.txt` 还是旧模板，或主程序里还残留“显示文本”动作；请更新模板并删除调试动作。
*   **图2（CSV 里写入了标题/按钮/表单文字）**：说明“追加到文件”接错变量；请确保只写入 `FinalCSVLine`。
