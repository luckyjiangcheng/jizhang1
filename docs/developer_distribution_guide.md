...
*   非标准的旧版“成功/取消通知”动作（后文会添加统一通知）

> 最终写入必须直接使用确认页返回的一行 CSV，避免写入 JSON 或网页文本。
...
如果您的 `ZenLedger.csv` 数据量非常大（例如超过 1000 行），仪表盘加载可能会变慢。请参考以下优化方法：

### 1. 限制读取的数据量（推荐）
在快捷指令读取 `ZenLedger.csv` 后，不要直接传给仪表盘。可以先通过以下动作过滤：
*   **获取文本的最后几行**: 例如只获取最后 300 行。这通常足以覆盖最近的趋势和分类分析。
*   **重要**: 过滤后的文本必须保留 CSV 的**表头行**（第一行），否则仪表盘无法解析。

### 2. 使用轻量化模板
确保您的 `dashboard.txt` 中的 ECharts 引用使用的是生产环境压缩版（CDN 链接），且尽量减少不必要的外部图片或脚本引用。

### 3. 本地化缓存
在快捷指令中，可以使用“检查文件是否存在”和“获取文件大小”来决定是否需要重新下载或处理数据。
...
### 1. 确认后显示一行 CSV 文本
说明设备上的 `confirm.txt` 还是旧模板，或主程序里还残留“显示文本”动作；请更新模板并删除调试动作。

### 2. CSV 里写入了标题/按钮/表单文字
说明“追加到文件”接错变量；请确保只写入 `FinalCSVLine`。

### 3. 仪表盘加载转圈或白屏
*   检查网络是否能正常访问 `cdn.jsdelivr.net`。
*   检查 `dashboard.txt` 内的 `CSV_DATA_PLACEHOLDER` 替换逻辑是否正确，确保没有破坏 HTML 结构。
...
5.  **在“🎙️ 语音记账”下**:
    ...
    *   **显示网页视图**:
        *   输入: 上一步的结果
        *   (用户点击“确认记账”会立即把 CSV 写入剪贴板并关闭页面；点击“取消”会直接关闭页面)
    *   **获取剪贴板**:
        *   (存为变量 **FinalCSVLine**)
